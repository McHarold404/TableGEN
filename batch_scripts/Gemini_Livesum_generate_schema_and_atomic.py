...
for idx in range(0,754):
    ...
    atomic_out = ask_gemini(text=df['text'][idx],prompt_path=f"prompts/Livesum/Current/livesum_atomic.txt",key = key,model_name = model_name)
    with open(f"./model_outputs/Livesum/{model_dir}/Atomic/{idx}.txt","w") as f:
        f.write(atomic_out)
    header_out = ask_gemini(text=atomic_out,prompt_path="prompts/Livesum/Current/livesum_header.txt", key = key ,model_name=model_name)
